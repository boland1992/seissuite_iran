...
for i in range(0, 1):   
  ...
  #      from seissuite.database import response_database
    ...
    #dates_pairs = list(it.combinations(dates, 2))
    #dates_pairs = list(it.chain(*dates_pairs))
    #print len(dates_pairs), len(dates)
    # create SNR dictionary to store SNR information throughout the stack time
    SNR_dict = {}
    data_dict = {}
    #SNR1, data1 = 0, 0

    
    # create list of unique combinations of dates. 
...
starttime <= ? AND endtime >= ?', (search_start, search_end))
        ...
        for pair in pairs: 
            ...
            s1, s2 = str(s1), str(s2)
           #print dir( xc[s1][s2])
            stat_string = '{}-{}'.format(s1, s2)
            dataarray = xc[s1][s2].dataarray
            timearray = xc[s1][s2].timearray
            SNR1 = np.max(dataarray) / np.std(dataarray)
            data1 = dataarray
            SNRarray = np.asarray(xc[s1][s2].SNR_lin)[:,0]
            print SNRarray
            if len(SNRarray) >= 2 and stat_string in data_dict.keys():
                SNR1, SNR2 = SNRarray[-2], SNRarray[-1]
                print SNR1, SNR2
            
                if SNR2 > SNR1:
                    xc[s1][s2].dataarray = data_dict[stat_string]
                print SNR1, SNR2
                    
            data_dict[stat_string] = dataarray
